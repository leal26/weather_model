...
for location in locations[1:5]:
    valid_data = True
    counter_x += 1
    page = ('http://weather.uwyo.edu/cgi-bin/sounding?region' 
            + '=naconf&TYPE=TEXT%3ALIST&YEAR=' + YEAR + '&MONTH=' + MONTH 
            + '&FROM=' + FROM + '&TO=' + TO + '&STNM=' + location)
    print(counter_x)
    # print(page)
    ...
    print(soup)
    # For loop finds two lines that start with '-' and starts processing 
    # there.  
    counter = 0
    ...
    if len(soup) > 100:
        ...
        # For loop will add new lines until finds a string 
        # starting with 'S'. 
        i = 0
        while soup[i][0] != 'S':
...
with open("WBData.csv", "r") as source:
    ...
    with open("WB" + YEAR + "_" + MONTH + "_" + DAY 
              + ".csv","a") as result:
        wtr= csv.writer(result)
        for r in rdr:
            wtr.writerow((r[0], r[1], r[3], r[2], r[4], r[6], 
                          r[8], r[9]))
